...
module.exports = async function(fastify) {
  ...
  fastify.get('/api/gallery', { schema: { tags: ['Gallery'] } }, async (req, reply) => {
    return db.prepare('SELECT * FROM gallery ORDER BY date DESC').all();
  });

  fastify.get('/api/gallery/:id', { schema: { tags: ['Gallery'], params: { type: 'object', properties: { id: { type: 'integer' } } } } }, async (req, reply) => {
    const item = db.prepare('SELECT * FROM gallery WHERE id = ?').get(req.params.id);
    if (!item) return reply.code(404).send({ error: 'Not found' });
    return item;
  });

  fastify.post('/api/gallery', { preHandler: [authMiddleware], schema: { tags: ['Gallery'], body: galleryBodySchema } }, async (req, reply) => {
  ...
  fastify.put('/api/gallery/:id', { preHandler: [authMiddleware], schema: { tags: ['Gallery'], body: galleryBodySchema, params: { type: 'object', properties: { id: { type: 'integer' } } } } }, async (req, reply) => {
  ...
  fastify.delete('/api/gallery/:id', { preHandler: [authMiddleware], schema: { tags: ['Gallery'], params: { type: 'object', properties: { id: { type: 'integer' } } } } }, async (req, reply) => {
